...
Linux 环境 10K 全量查询实测（beam=8, max-visits=1000, 无缓存, 无预取）：

**消融实验（8 组全组合）：**

| 配置 | QPS | avg(ms) | P99(ms) | Recall@10 | cache_hit |
|------|-----|---------|---------|-----------|-----------|
| t1, cache0 | 113.7 | 8.7 | 12.1 | 0.9897 | — |
| t1, cache0, prefetch | 52.1 | 19.1 | 138.4 | 0.9897 | — |
| t1, cache200 | 117.7 | 8.4 | 11.9 | 0.9897 | 46.0% |
| t4, cache0 | 232.8 | 17.1 | 90.3 | 0.9897 | — |
| t8, cache0 | 321.5 | 24.8 | 36.3 | 0.9897 | — |
| t4, cache200 | 228.2 | 17.4 | 98.8 | 0.9897 | 46.0% |
| t4, cache200, prefetch | 215.7 | 18.4 | 39.5 | 0.9897 | 45.0% |

**缓存容量对比（t4, 10K queries）：**

| 缓存 | QPS | P99(ms) | 命中率 |
|------|-----|---------|--------|
| 0 MB | 80.0 | 287.2 | — |
| 2 MB | 171.5 | 38.1 | 1.3% |
| 20 MB | 178.6 | 37.4 | 1.8% |
| 80 MB | 243.1 | 44.3 | 19.1% |
| 200 MB | 260.7 | 58.5 | 46.0% |

**搜索参数敏感性（t4, 10K queries, 无缓存）：**

| beam | visits=500 | 1000 | 2000 |
|------|-----------|------|------|
| 4 | 0.9679 / 300 QPS | 0.9880 / 271 QPS | 0.9946 / 218 QPS |
| 8 | 0.9723 / 290 QPS | **0.9897** / **252 QPS** | 0.9959 / 204 QPS |
| 16 | 0.9738 / 291 QPS | 0.9904 / 243 QPS | 0.9961 / 190 QPS |
| 32 | 0.9749 / 280 QPS | 0.9911 / 224 QPS | 0.9963 / 168 QPS |
...
> 关键结论：(1) 预取在 SSD 上始终为负优化，单线程下 QPS 减半，已默认关闭；(2) 缓存 200MB 命中率 46%，单线程场景下有约 3.5% 的 QPS 提升；(3) 多线程是最大加速来源，8 线程达峰值 QPS 321.5（3.0×）；(4) Recall 在所有配置下完全一致，验证各优化只影响 I/O 路径。
