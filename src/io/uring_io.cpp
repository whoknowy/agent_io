...
UringIOBackend::UringIOBackend(size_t queue_depth, bool sqpoll, bool writable)
    : queue_depth_(queue_depth), sqpoll_(sqpoll), writable_(writable) {
  ring_ = new io_uring;
  unsigned flags = 0;
  if (sqpoll_) {
...
bool UringIOBackend::Open(const std::string& path) {
  ...
  int flags = writable_ ? (O_RDWR | O_CREAT) : O_RDONLY;
  mode_t mode = writable_ ? 0644 : 0;
...
bool UringIOBackend::WriteAt(uint64_t offset, const void* buf, size_t size) {
  if (fd_ < 0 || !ring_init_ || !writable_) return false;

  io_uring_sqe* sqe = io_uring_get_sqe(ring_);
  if (!sqe) return false;

  io_uring_prep_write(sqe, fd_, buf, static_cast<unsigned>(size), offset);
